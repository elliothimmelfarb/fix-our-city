...
export default (app) => {
  ...
  // the client connected through the front-facing proxy, or series of proxies.
  ...
  // The trust proxy setting is implemented using the proxy-addr
  ...
  // Options: resave: forces the session to be saved back
  // to the session store, even if the session was never
  ...
  // to the session in one request may get overwritten when the
  // other request ends, even if it made no
  // changes(this behavior also depends on what store you're using).
  // saveUnitialized: Forces a session that is uninitialized to
  // be saved to the store. A session is uninitialized when
  // it is new but not modified. Choosing false is useful for
  // implementing login sessions, reducing server storage
  // usage, or complying with laws that require permission
  //  before setting a cookie. Choosing false will also help with
